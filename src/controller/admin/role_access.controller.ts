import { Body, Controller, Param, Get, Put } from '@nestjs/common';
...
import { updateRoleAccess } from '../../model/errorInfo';
import { ADMIN } from '../../config/routerPrefix';

@Controller(ADMIN + '/role/access')
export class RoleAccessController {
  constructor(private readonly service: RoleAccessService, private readonly accessService: AccessService) {}
  ...
  async getRoleAccessByRoleId(@Param('id') id: string) {
    const [access, ids] = await Promise.all([
      this.accessService.getAccessThree(),
